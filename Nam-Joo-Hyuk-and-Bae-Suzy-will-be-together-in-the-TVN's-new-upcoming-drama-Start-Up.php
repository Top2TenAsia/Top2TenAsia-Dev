
<!DOCTYPE html>
<html lang="en">
<head>
<!-- Global site tag (gtag.js) - Google Analytics -->
<script async src="https://www.googletagmanager.com/gtag/js?id=UA-160757768-2"></script>
<script>
  window.dataLayer = window.dataLayer || [];
  function gtag(){dataLayer.push(arguments);}
  gtag('js', new Date());

  gtag('config', 'UA-160757768-2');
</script>

<script data-ad-client="ca-pub-2272028859368526" async src="https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js"></script>
  <title>Nam Joo Hyuk and Bae Suzy will be together in the TVN's new upcoming drama Start-Up</title>
  
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.4.1/css/bootstrap.min.css">
  <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.4.1/jquery.min.js"></script>
  <script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.4.1/js/bootstrap.min.js"></script>


   <meta charset="UTF-8">
   <meta property="og:site_name" content="Top2Ten"/>
   <meta property="og:title" content="Nam Joo Hyuk and Bae Suzy will be together in the TVN's new upcoming drama Start-Up"/>
<link rel="canonical" href="/Nam-Joo-Hyuk-and-Bae-Suzy-will-be-together-in-the-TVN's-new-upcoming-drama-Start-Up" />
  <meta name="description" content="Nam Joo Hyuk and Bae Suzy will be together in the TVN's new upcoming drama Start-Up" />
<meta name="keywords" content="Start-Up, start-up, startup, start up, Nam Joo Hyuk,Bae Suzy, new drama, new korean drama, korean drama, kang hana, kim seon, tvN, TVN's new drama">           
  <meta name="owner" content="Harish Panwar">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <style>
  .test{
      width:40%;
  }
  h1{
         font-size:28px;
         font-family:calibri;
         letter-spacing:0.2ex;
     }
  h2 {
    font-size:22px;
    font-family:calibri;
    letter-spacing:0.2ex;
   
  }
  h3{
      font-size:18px;
      font-family:calibri;
      letter-spacing:0.2ex;

  }
 @media only screen and (max-width: 600px) {
     h1{
         font-size:19px;
         font-family:times new roman;
     }
  h2 {
    font-size:18px;
    font-family:times new roman;
   
  }
  h3{
      font-size:17px;
      
      word-spacing:0.5ex;
     font-family:times new roman;
  }
 .test{
      width:100%;
  }
}
html {
  scroll-behavior: smooth;
}
 a{
  color: coral;
}
</style>
<link rel="apple-touch-icon" sizes="180x180" href="/assets/apple-touch-icon.png">
<link rel="icon" type="image/png" sizes="32x32" href="/assets/favicon-32x32.png">
<link rel="icon" type="image/png" sizes="16x16" href="/assets/favicon-16x16.png">
<link rel="manifest" href="/site.webmanifest">
</head>
<body>
<?php include("header.php") ?>

<br>
<center style="background-color: coral;margin-top:-2.6ex;"><p id="demo" style="color: white;font-size: 25px;"></p></center>
<center><img src="assets/head.webp" class="img-thumbnail test" ></center>
  <center><h1>Nam Joo Hyuk and Bae Suzy will be together in the TVN's new upcoming drama Start-Up</h1></center>
  
  
  <h2 style="text-align:center;color: orange">
<span class="fa fa-star checked"></span>
<span class="fa fa-star checked"></span>
<span class="fa fa-star checked"></span>
<span class="fa fa-star checked"></span> 
<span class="fa fa-star checked"></span>
<span class="fa fa-star checked"></span>
<span class="fa fa-star checked"></span>
<span class="fa fa-star checked"></span>
<span class="fa fa-star checked"></span>
<span class="fa fa-star checked"></span></h2>
<br>
<div class="container">
<p><i class="fa fa-clock-o"></i> 8:00 PM Apr 27, 2020 IST</p> <br><br>
<div class="row" >
    <a href="https://www.facebook.com/Top2Ten-105271447753021/" target="blank">
    <div class="col-md-6" style="background-color: #3b5998;font-family: Times new roman;height: 6ex;padding: 1.3ex;color: white;border:  3px solid white;">
      <center>
      Follow us on Facebook <i class="fa fa-facebook-square"></i>
    </center>
  </div> </a>
  <a href="https://www.instagram.com/top2ten.asia/" target="blank">
  <div class="col-md-6" style="background-color: #e4405f;font-family: Times new roman;height: 6ex;padding: 1.3ex;color: white;border:  3px solid white;">
    <center>
    Follow us on Instagram <i class="fa fa-instagram"></i>
  </center>
  </div>
</a>
</div><br>
<a href="https://www.facebook.com/sharer/sharer.php?u=https://www.top2tenasia.xyz/The-World-Of-The-Married-is-few-ratings-away-to-make-history-on-cable-network" target="_blank" class="fa fa-facebook" style="color:#3b5998">Share on facebook</a>
<p> By <a href="https://www.facebook.com/harishpanwar1998">Harish Panwar</a></p><br>
<br>



<h3 style="text-align:center"> Hey Chingus!! good news for those Korean Drama lovers who are getting bored at home and waiting for the drama with good star cast. Our beloved and one of the dream onscreen couples <a href="https://www.google.com/search?gs_ssp=eJzj4tVP1zc0TDJLMyjJSS4zYPTiyUvMVcjKz1fIqCzNBgCBJwky&q=nam+joo+hyuk&oq=na&aqs=chrome.1.69i59j46j69i59j69i57j35i39j69i60l3.2395j0j9&sourceid=chrome&ie=UTF-8">Nam Joo Hyuk</a> popularly known for the Korean Drama Weightlifting Fairy Kim Bok Joo and <a href="https://www.google.com/search?gs_ssp=eJzj4tLP1TfIMM-tMsw2YPTiSEpMVSguraoEAEnzBtg&q=bae+suzy&oq=bae+sux&aqs=chrome.2.69i57j35i39j46j0l5.2957j0j9&sourceid=chrome&ie=UTF-8">Bae Suzi</a> popularly know for the Korean Drama While You Were Sleeping) are going to be together in the TVN's new upcoming drama Start-Up Formerly known as "Sandbox".</h3>
<br>
<h2>About Drama</h2>
<h3><center><span style="font-weight:bold">Network:</span> The drama will be aired on tvN network<br><br>
  <span style="font-weight:bold">Episodes:</span> 16<br><br>

<span style="font-weight:bold">Genres:</span> Start-Up have  Business, Comedy, Romance, Drama genres <br><br>



<span style="font-weight:bold">Airing on:</span>  The Drama will be aired from Oct 10, 2020 to Nov 29, 2020<br><br>

<span style="font-weight:bold">Airtime:</span> The airtime is on weekends Saturday and Sunday)<br><br>

<span style="font-weight:bold">Screenwriters:</span> Screenwriting is done by Park Hye Ryun<br><br>

<span style="font-weight:bold">Directors:</span>  Startup is directed by Oh Choon Hwan</center></h3>
<br><br>
<h2 style="text-decoration:underline">Synopsis</h2>
<h3>
Startup as we can guess from the title it's basically based on the story of people who want to start up their own business.

It is a story of two entrepreneurs who have a dream and want to get success with their own start-up companies. Seo Dal-mi (Bae Suzy), a second-generation heiress who drops out of the college just like the Facebook owner Mark Zuckerberg to start her own company and be as successful as the famous entrepreneurs like Bill Gates, Steve Jobs, Mark Zuckerberg, etc. She also gives up her all privileges and takes various jobs to raise the money she can use as capital. She wants to create her startup from the scratch.<br><br>

On the other side Nam Do-san, the founder and CEO of a failing start-up company. He excelled at Math as a child and a genious who constantly made his family proud, but since running his own company two years ago, life has become difficult for him. He is working hard to make his startup company successful.

When one day Nam Do-san suddenly meet up with Seo Dal-mi, the two become good friends and offer each other support as they try to launch a next and new big start-up that will make them successful.
</h3>
  <h2 style="text-decoration:underline; ">Cast</h2>  <br>                                             
  <center style="color:#3b5998;font-weight:bold;font-size:18px;">Nam Joo Hyuk will be seen as Nam Do San</center>
  <center><img src="assets/nam.png" class="img-thumbnail test"></center><br>
  <center style="color:#3b5998;font-weight:bold;font-size:18px;">Bae Suzy will be seen  as  Seo Da Mi</center>
  <center><img src="assets/suzy.webp" class="img-thumbnail test"></center><br>                                 
  <center style="color:#3b5998;font-weight:bold;font-size:18px;">Kang Hana will be seen as Won In Jae</center>
  <center><img src="assets/Kang.jpg" class="img-thumbnail test"></center><br>
  <center style="color:#3b5998;font-weight:bold;font-size:18px;">Kim Seon Ho will be seen as Han Ji Pyung</center>
  <center><img src="assets/kim.webp" class="img-thumbnail test"></center>
<br><br>
<h3>It will so exciting to see Nam Joo Hyuk and Bae Suzy who are one of the most well known and famous Korean Drama actors to work together and also it will be new experience for the viewers as they are together in a drama for the first time. I assure you chingus it will be good.
<br>
                                                   <h3 style="colour:coral">Other Popular Posts</h3>
    <hr>


    <div class="row" style="margin-top:5ex;border: 1px solid #ccc;border-radius: 4px;" >
    <div class="col-md-5"> 
    <a href="/Lee-Joon-Gi-Is-Back-With-His-New-Upcoming-tvN-Thriller-Drama">
    <img src="assets/flower.webp" class="img-thumbnail" style="border-radius: 1ex;" >
    </a>
    </div>
    <div class="col-md-7" >
    <a href="/Lee-Joon-Gi-Is-Back-With-His-New-Upcoming-tvN-Thriller-Drama">

      <h2>
      Lee Joon Gi Is Back With His New Upcoming tvN Thriller Drama [Top2Ten]</h2>
              </a>
              <h3> Lee Joon Gi and Moon Chae Won will be together on Upcoming tvN Thriller, once again they are together and the viewers is eagerly waiting for the drama.</h3>
                <p><i class="fa fa-clock-o"></i> 8:00 PM Apr 30, 2020</p> 
      </div>
            </a>
    </div>
    <div class="row" style="margin-top:5ex;border: 1px solid #ccc;border-radius: 4px;" >
    <div class="col-md-5"> 
    <a href="best-korean-drama-from-each-year">
    <img src="assets/king.jpg" class="img-thumbnail" style="border-radius: 1ex;" >
    </a>
    </div>
    <div class="col-md-7" >
    <a href="best-korean-drama-from-each-year">

      <h2>
        Best korean drama from each year [Top2Ten]</h2>
              </a>
              <h3> For every year there can be only one drama that sits on the crown to be called as Drama of the year. This is the list of a masterpiece from every year, containing the top drama of each year from 2009-2019</h3>
                <p><i class="fa fa-clock-o"></i> 10:00 AM Apr 16, 2020</p> 
      </div>
            </a>
    </div>
        
        <div class="row" style="margin-top:2.5ex;border: 1px solid #ccc;border-radius: 4px;">
    
    <div class="col-md-5">
        <a href="top-10-best-korean-drama-of-the-decade">
      <img src="assets/k-drama.jpg" class="img-thumbnail" style="border-radius: 1ex;" >
            </a>
            </div>
            <div class="col-md-7">
              <a href="top-10-best-korean-drama-of-the-decade">
      <h2>
        Top 10 best Korean drama of decade [Top2Ten] </h2>
                </a>
        <h3>The popularity of korean drama is on it's peak, In the past 10 years the korean drama popularity is exploading all around the world, now the audience is not only limited to Asia but from all around the world. With having billons of k-drama lovers from all around the world k-drama industry plays one of the key role in the field of entertainment and South Korean economy. So here is the list of top 10 best K-drama of the decade that you must watch.</h3>  <p><i class="fa fa-clock-o"></i> 2:00 PM Apr 8, 2020</p>
      </div>
            
</div>
</div>
<?php include("footer.php") ?>
</body>
</html>